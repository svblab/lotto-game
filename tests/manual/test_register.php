...
echo "=== STARTING MANUAL TESTING FOR EPIC-1.0 ===\n\n";
...
try {
    ...
    $res1 = $authService->register('test_user_1', 'password123');
    if (isset($res1['success']) && $res1['success'] === true && $res1['username'] === 'test_user_1') {
    ...
    try { 
        $authService->register('test_user_1', 'password123');
        echo "❌ Failure: Duplicate user was allowed!\n";
        exit(1);
    } catch (\Exception $e) {
        if ($e->getMessage() === 'Username already exists') {
            echo "✅ Success: Correctly rejected duplicate username.\n";
        } else {
    ...
    try { 
        ...
        echo "❌ Failure: Short username was accepted!\n";
        exit(1);
    } catch (\Exception $e) {
        echo "✅ Success: Formally caught validation exception: " . $e->getMessage() . "\n";
    }
    echo "----------------------------------------\n";
    ...
    try { 
        $authService->register('test_user_2', '123');
        echo "❌ Failure: Short password was accepted!\n";
        exit(1);
    } catch (\Exception $e) {
        echo "✅ Success: Formally caught validation exception: " . $e->getMessage() . "\n";
    }
    ...
    echo "[Scenario 5] Checking database fields for default values (coins = 500)...\n";
    $checkStmt = $pdo->prepare("SELECT username, coins FROM users WHERE username = 'test_user_1'");
    $checkStmt->execute();
    $userRow = $checkStmt->fetch();
    
    if ($userRow && (int)$userRow['coins'] === 500) {
        echo "✅ Success: Initial balance is strictly 500 coins.\n";
    } else {
        echo "❌ Failure: Incorrect or missing initial data. Balance: " . ($userRow['coins'] ?? 'null') . "\n";
        exit(1);
    }
    echo "----------------------------------------\n";
